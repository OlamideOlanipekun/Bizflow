...
const Dashboard: React.FC = () => {
  ...
  const handleQuickAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let customerName = quickTx.customerName;
      ...
      if (!customerId) {
        showNotification("Please select or enter a customer.");
        return;
      }

      await api.addTransaction({
        customerId: customerId,
        customerName: customerName || customers.find(c => c.id === customerId)?.name || 'Unknown',
      ...
      showNotification("Failed to record event.");
    }
  };
  ...
  if (error || !stats) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center space-y-8 animate-fade-in">
        <div className="w-24 h-24 bg-rose-50 rounded-[2.5rem] flex items-center justify-center text-rose-500 shadow-xl shadow-rose-100">
        ...
        <div className="text-center space-y-3">
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter">System Desync</h2>
          <p className="text-slate-500 font-medium max-w-md">{error || "Connection to the secure data node timed out. Please verify your identity and retry."}</p>
        </div>
        <button
          onClick={() => fetchData()}
          className="px-10 py-5 bg-slate-950 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-2xl"
        >
          Retry Sync
        </button>
      </div>
    ...
    <div className="max-w-[1700px] mx-auto space-y-12 pb-24 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        <StatCard
          ...
          color="bg-indigo-50"
          sparklineData={stats.sparklines?.revenue || [0, 0, 0, 0, 0, 0, 0]}
          sparklineColor="#4f46e5"
        />
        <StatCard
          title="Account Expansion"
          value={stats.totalCustomers}
          change={`${stats.monthlyGrowth > 0 ? '+' : ''}${Math.round(stats.monthlyGrowth * 0.8)}%`}
          icon={<ICONS.Customers className="w-7 h-7 text-emerald-600" />}
          color="bg-emerald-50"
          sparklineData={stats.sparklines?.customers || [0, 0, 0, 0, 0, 0, 0]}
          sparklineColor="#10b981"
        />
        <StatCard
          title="Conversion Velocity"
          value={`${stats.totalTransactions} total`}
          change={`${stats.monthlyGrowth > 0 ? '+' : ''}${(stats.monthlyGrowth * 1.2).toFixed(1)}%`}
          icon={<ICONS.Bolt className="w-7 h-7 text-amber-600" />}
          color="bg-amber-50"
          sparklineData={stats.sparklines?.transactions || [0, 0, 0, 0, 0, 0, 0]}
            ...
            <div className="bg-white p-12 rounded-[4rem] shadow-sm border border-slate-100">
              ...
              <div className="h-72 relative">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={stats.revenueByCategory || [{ name: 'N/A', value: 1 }]}
                      cx="50%"
                      cy="50%"
                      innerRadius={95}
                      outerRadius={120}
                      ...
                      dataKey="value"
                    >
                      {(stats.revenueByCategory || [{ name: 'N/A', value: 1, color: '#f1f5f9' }]).map((entry: any, index: number) => (
                        <Cell key={`cell-${index}`} fill={entry.color || ['#4f46e5', '#10b981', '#f59e0b', '#f43f5e', '#8b5cf6'][index % 5]} stroke="none" cornerRadius={16} />
                      ))}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                ...
                <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                  <span className="text-5xl font-black text-slate-900 leading-none">{stats.revenueByCategory?.length ? '100%' : '0%'}</span>
                  <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-3 opacity-60">Coverage</span>
                </div>
              </div>
              <div className="mt-14 space-y-4">
                {(stats.revenueByCategory || []).map((d: any, i: number) => (
                  <div key={d.name} className="flex items-center justify-between p-5 bg-slate-50 rounded-3xl hover:bg-white hover:shadow-2xl hover:scale-[1.03] transition-all cursor-default border border-transparent hover:border-slate-100 group">
                    <div className="flex items-center gap-4">
                      <div className="w-3.5 h-3.5 rounded-full shadow-sm" style={{ backgroundColor: d.color || ['#4f46e5', '#10b981', '#f59e0b', '#f43f5e', '#8b5cf6'][i % 5] }}></div>
                      <span className="text-sm font-black text-slate-700">{d.name}</span>
                    </div>
                    <span className="text-sm font-black text-slate-900 group-hover:text-indigo-600 transition-colors">${d.value.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-3xl rounded-[5rem] shadow-[0_0_120px_rgba(0,0,0,0.6)] overflow-hidden animate-scale-in border border-white/20">
            <div className="p-16 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="space-y-2">
                <h2 className="text-5xl font-black text-slate-900 tracking-tighter leading-none">Add Transaction</h2>
                <p className="text-slate-500 font-medium text-lg">Enter a new financial entry for your records.</p>
              </div>
              <button
            ...
            <form onSubmit={handleQuickAdd} className="p-20 space-y-12">
              <div className="grid grid-cols-2 gap-12">
                <div className="col-span-2">
                  <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mb-5">Customer Name</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mb-5">Transaction Amount ($)</label>
                  <input
                    required
                    type="number"
