      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10">
        <div>
          <div className="flex items-center gap-3 mb-4">
             ...
             <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">Immutable Ledger Node</span>
          </div>
          <h1 className="text-6xl font-black text-slate-900 tracking-tighter leading-none">Financial Ledger</h1>
        ...
        <div className="flex items-center gap-6">
          ...
          >
            ...
            <span>{isExporting ? 'Generating Audit...' : 'Audit Export'}</span>
          </button>
      ...
      <div className="bg-white rounded-[4rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-14 border-b border-slate-100 flex flex-col xl:flex-row gap-12 justify-between items-center bg-slate-50/20">
          <div className="flex bg-white p-2.5 rounded-[2.5rem] shadow-sm border border-slate-100 flex-shrink-0">
            {['All', 'Completed', 'Pending', 'Cancelled'].map((status) => (
              <button
                ...
                className={`px-10 py-4 rounded-[1.75rem] text-[11px] font-black uppercase tracking-[0.25em] transition-all ${
                  filter === status 
          ...
          <div className="relative w-full xl:w-[550px]">
            ...
            <input 
              type="text" 
              placeholder="Query by Trace ID, sector, or principal..." 
              value={search}
                    ...
                    <td className="px-14 py-8 text-right">
                      <div className="flex items-center justify-end gap-4">
                        <div className={`w-3 h-3 rounded-full ${
                          tx.status === 'Completed' ? 'bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.5)]' :
